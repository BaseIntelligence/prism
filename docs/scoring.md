...
PRISM scores one thing: a model's ability to learn from scratch, measured as online compression. The
primary metric is a **prequential bits-per-byte (bpb)** score the challenge computes itself from a
forced-init re-execution; a held-out delta breaks near-ties and an anti-memorization gap penalizes
overfitting. **Lower bits-per-byte is better.**
