# PRISM mamba-tiny ~1M-parameter pure-PyTorch Mamba/SSM training script.
#
...
# a single-pass, challenge-controlled predict-then-train instrument. The loop
# never opens files or touches the network and never imports blocked native
# mamba_ssm extensions.
#
# Multi-GPU: single-node ≤8 safe (init_process_group, set_device(local_rank),
# DDP wrap, DistributedSampler marker, rank-0 save, destroy_process_group). Also
# works at world_size=1 (scored nproc=1).
import torch
...
# Slightly conservative LR vs transformer tiny-1m for pure-torch SSM scan stability.
EFFECTIVE_LEARNING_RATE = 0.003
GRAD_CLIP_NORM = 1.0
LOCAL_BATCH = 4
...
def train(ctx: PrismContext) -> None:
    ...
    # directly at ctx.artifacts_dir so the sandbox recognizes it as a trusted write.
